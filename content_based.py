...
content_based.py
-----------------
Content-based recommender: recommends movies similar to a given movie,
based on overview + genres + keywords + tagline ("soup"). (Cast/director are
left as optional empty fields in this simplified pipeline — see README if
you want to add them back in later via the TMDB API.)

Method: CountVectorizer (bag-of-words) + cosine similarity.
(Swap CountVectorizer for TfidfVectorizer to compare methods for your report.)
"""
...
class ContentBasedRecommender:
    ...
    def recommend(self, title: str, top_n: int = 10) -> pd.DataFrame:
        ...
        if isinstance(idx, pd.Series):  # duplicate titles guard
            idx = idx.iloc[0]
